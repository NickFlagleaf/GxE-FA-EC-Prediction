...
# Genotype marker matrix pruning function
# Removes one of each of pairs of linked markers above a defined linkage threshold

SNPprune <- function(marker.mat, # N x M marker matrix with row names and column names. Markers coded as 0,1 and 2 for hom, het and hom.
    ...
    for (i in 1:ncol(marker.mat)) { # For each marker in order
      if (!colnames(marker.mat)[i] %in% to.remove) { # If the marker isn't already assigned for removal:
        ...
        if (length(tocheck) > 1) {
          ...
          c.sub <- apply(pruned[, tocheck], 2, function(x) {
            cor(x, pruned[, colnames(marker.mat)[i]],
