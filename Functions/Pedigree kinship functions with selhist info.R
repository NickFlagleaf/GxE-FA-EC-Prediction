...
Expand.ped<-function(ped, #A data frame of pedigree data with columns:
                                                #'GID' - Genotype ID names character vector
                                                #'Cross' - pedigree string in Purdy format character vector
  ...
  all.gens<-list()
  if(is.null(max.depth)){max.depth<-100}
  cat("|",sep="")
  ...
  #Add in extra generations for BC parent 1s
  if(sum(isP1BC)>0){
    ...
    recrnt.spltP1<-lapply(1:length(P1.split.pos),function(x) c(substr(parents.mat[isP1BC,"P1"][x],1,P1.split.pos[x]-1),
    ...
    parents.mat[isP1BC,"P2"]<-paste(parents.mat[isP1BC,"P2"],newsep,
                                    paste(newBClvlsP1,apply(as.matrix(1:length(recrnt.spltP1)),1,function(x) recrnt.spltP1[[x]][!is.no.BCP1[[x]]]),sep=""),sep="")
  }
  ...
  if(sum(isP2BC)>0){
    ...
    recrnt.spltP2<-lapply(1:length(P2.split.pos),function(x) c(substr(parents.mat[isP2BC,"P2"][x],1,P2.split.pos[x]-1),
    ...
    parents.mat[isP2BC,"P1"]<-paste(parents.mat[isP2BC,"P1"],newsep,
                                    paste(newBClvlsP2,apply(as.matrix(1:length(recrnt.spltP2)),1,function(x) recrnt.spltP2[[x]][!is.no.BCP2[[x]]]),sep=""),sep="")
  }
  ...
  while(donextlevel==T){
    ...
    }else{parents.mat<-matrix(NA,nrow=0,ncol=2,dimnames = list(NULL,c("P1","P2")))}
    
   if(!length(newIDs)>0){break}
    
    isP1BC<-(grepl(pattern = c("\\*"),
                   x = str_sub(parents.mat[,"P1"],2,2))&str_sub(parents.mat[,"P1"],1,1)%in%as.character(1:9)) | (grepl(pattern = c("\\*"),
                                                                                                                       x = str_sub(parents.mat[,"P1"],-2,-2))&str_sub(parents.mat[,"P1"],-1,-1)%in%as.character(1:9))
    if(sum(isP1BC)>0){
      P1.split.pos<-sapply(parents.mat[isP1BC,"P1"],function(x) unlist(gregexpr(pattern = "\\*",x))[unlist(gregexpr(pattern = "\\*",x)) %in% c(2,str_length(x)-1)])
      recrnt.spltP1<-lapply(1:length(P1.split.pos),function(x) c(substr(parents.mat[isP1BC,"P1"][x],1,P1.split.pos[x]-1),
                                                                 substr(parents.mat[isP1BC,"P1"][x],P1.split.pos[x]+1,str_length(parents.mat[isP1BC,"P1"][x]))))
    ...
    isP2BC<-(grepl(pattern = c("\\*"),
                   x = str_sub(parents.mat[,"P2"],2,2))&str_sub(parents.mat[,"P2"],1,1)%in%as.character(1:9)) | (grepl(pattern = c("\\*"),
                                                                                                                       x = str_sub(parents.mat[,"P2"],-2,-2))&str_sub(parents.mat[,"P2"],-1,-1)%in%as.character(1:9))
    ...
    if(length(all.gens)>max.depth){donextlevel<-FALSE}
  }
...
Get.kin<-function(expnd.ped, #Expanded pedigree in three column format output from Expand.ped function
  ...
  if(is.null(IDs)){IDs<-unique(c(expnd.ped$ID,expnd.ped$P1,expnd.ped$P2))}
  ...
  expnd.ped<-data.frame("ID"=c(expnd.ped$ID,orphns),
  ...
  all.selhist.inbreds<-sapply(1:length(selhists.list),function(i){ #Add in intermediate generation in the selection histories
                                  selhist.inbreds<-data.frame("ID"=NA,
