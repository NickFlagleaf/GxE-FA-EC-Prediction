...
FA.nxt.SVs <- function(old.vparams,
  ...
  if (length(missingpckgs) > 0) {
    print(paste("Install packages:", paste(missingpckgs, collapse = " ")))
  }
  ...
  names(old.fas) <- gsub(
    pattern = paste(old.fa.add.order, "):", add.fa.str, sep = ""),
    replacement = paste(new.add.FA.order, "):", add.fa.str, sep = ""), names(old.fas)
  ) # update the additive fa order number to new one
  names(old.fas) <- gsub(
    pattern = paste(old.fa.nonadd.order, "):", non.add.fa.str, sep = ""),
    replacement = paste(new.nonadd.FA.order, "):", non.add.fa.str, sep = ""), names(old.fas)
  ) # update the non-additive fa order number to new one
