rfSNPimpute <- function(marker.mat, # NxM marker matrix with row names and column names. Markers coded as 0,1 and 2 for hom, het and hom.                      n.cores = NULL, # number of cores to use
...

{
  ...
  if (verbose == T) {
    print("Imputing log is output in directory")
  }
  ...
  imputed <- foreach::foreach(i = 1:ncol(marker.mat), .combine = cbind, .multicombine = TRUE, .packages = "ranger", .verbose = F) %dopar% { # for each marker:
    ...
    if (isTRUE(!round((1:ncol(marker.mat) / ncol(marker.mat)) * 100)[i] == round((1:ncol(marker.mat) / ncol(marker.mat)) * 100)[i - 1])) {
      if (verbose == T) {
        print(paste("|", round((1:ncol(marker.mat) / ncol(marker.mat)) * 100)[i], "%", sep = ""))
