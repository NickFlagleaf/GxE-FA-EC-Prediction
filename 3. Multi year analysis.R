...
if (length(env.ranefs) > 0) {
  DIAGranform <- paste(genetic.form, paste(unlist(lapply(1:length(env.ranefs), function(x) {
    paste("at(Env,'", names(env.ranefs)[x], "'):", env.ranefs[[x]], "+", sep = "")
  })), collapse = ""))
...
if (lmmod.diag$converge == F) {
  lmmod.diag <- update(lmmod.diag)
} # make sure model converged ok
...
if (length(env.ranefs) > 0) {
  FA1ranform <- paste(
    "~rr(Env,1):vm(GID,A)+diag(Env):vm(GID,A)+",
...
if (lmmod.fa1.fa1$converge == F) {
  lmmod.fa1.fa1 <- update(lmmod.fa1.fa1)
} # make sure model converged ok
...
if (length(env.ranefs) > 0) {
  FA2ranform <- paste(
    "~rr(Env,2):vm(GID,A)+diag(Env):vm(GID,A)+",
    paste(unlist(lapply(1:length(env.ranefs), function(x) paste("at(Env,'", names(env.ranefs)[x], "'):", env.ranefs[[x]], "+", sep = ""))), collapse = "")
  )
...
sv.for.FA2 <- FA.nxt.SVs(
  old.vparams = lmmod.fa1.fa1$vparameters,
  new.default.SVs = fa2.start$vparameters.table,
  new.add.FA.order = 2
)

# Fit FA2 model
lmmod.fa2.fa1 <- asreml(
  fixed = Grain.Yield..t.ha. ~ Env,
  random = as.formula(FA2ranform),
  residual = as.formula(resid.form),
  G.param = sv.for.FA2,
...
if (lmmod.fa2.fa1$converge == F) {
  lmmod.fa2.fa1 <- update(lmmod.fa2.fa1)
} # make sure model converged ok
...
if (length(env.ranefs) > 0) {
  FA3ranform <- paste(
    "~rr(Env,3):vm(GID,A)+diag(Env):vm(GID,A)+",
    paste(unlist(lapply(1:length(env.ranefs), function(x) paste("at(Env,'", names(env.ranefs)[x], "'):", env.ranefs[[x]], "+", sep = ""))), collapse = "")
  )
...
sv.for.FA3 <- FA.nxt.SVs(
  old.vparams = lmmod.fa2.fa1$vparameters,
  new.default.SVs = fa3.start$vparameters.table,
  new.add.FA.order = 3
)

# Fit FA3 model
lmmod.fa3.fa1 <- asreml(
  fixed = Grain.Yield..t.ha. ~ Env,
  random = as.formula(FA3ranform),
  residual = as.formula(resid.form),
  G.param = sv.for.FA3,
...
if (lmmod.fa3.fa1$converge == F) {
  lmmod.fa3.fa1 <- update(lmmod.fa3.fa1)
} # make sure model converged ok
...
# Make summary stats across models
models.comp <- data.frame(
  "nParams" = unlist(lapply(all.mods, function(x) length(x$vparameters))),
