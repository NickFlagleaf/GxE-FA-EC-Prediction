...
#' All.best.mods' is a list of the best models for each environment identified by the '1. Single trial analysis' script
...
asreml.options(workspace = "10028mb", trace = TRUE, maxit = 200, ai.sing = TRUE) # Set required memory up ASREML workspace
...
for (i in 1:length(unique(plot.data$Year))) { # for all the trials in each year
  ...
  if (length(env.ranefs) > 0) {
    FA1ranform <- paste(
      "~rr(Env,1):vm(GID,Asub)+diag(Env):vm(GID,Asub)+", # including FA and environment specific (diag) genotype effects
