...
#Make empty lists to fill with model fitted outputs for each trial
...
for(i in 1:length(envs)){ #For each trial environment
  ...
  Rcovs<-list("GRC"=as.formula(~GID+ROW+COL),
  ...
  Rescovs<-list("idar1"=as.formula(~id(ROW):ar1(COL)),
  ...
  for(R in 1:length(Rcovs)){
    ...
    asmod<-asreml(fixed = Grain.Yield..t.ha.~1,
                  random = Rcovs[[R]],
                  residual = Rescovs[[r]],
                  data=trial_sub,
  ...
  if(Rcovs.noGID[[which.R.cov]]=="~GID"){
    fixasmod<-asreml(fixed = Grain.Yield..t.ha.~1+GID,
                     data=trial_sub,
                     na.action = list(x = "include", y = "include"))
    max.updates<-10
  ...
  Gpred$BLUEs<-blues
  Gpred<-cbind("Year"=substr(envs[i],1,4),
