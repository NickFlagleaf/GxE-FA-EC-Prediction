...
main.eff.OOF.preds <- matrix(NA,
  nrow = length(env.maineffs), ncol = 3,
...
for (i in 1:nrow(Lam)) {
  ...
  # LASSO
  cv.mod <- cv.glmnet(
    x = WSmat[rownames(Lam), ][-i, ], y = env.maineffs[-i], # CV LASSO model to find optimum lambda
    nfolds = 10, lambda = seq(0.0001, 0.6, length.out = 100)^2
  )
  lassso.mod <- glmnet(
    x = WSmat[rownames(Lam), ][-i, ], y = env.maineffs[-i], # Fit LASSO model
  ...
  for (f in 1:ncol(Lam)) { # For each factor separately
    ...
    # LASSO
    cv.mod <- cv.glmnet(
      x = WSmat[rownames(Lam), ][-i, ], y = Lam[-i, f], # CV LASSO model to find optimum lambda
      nfolds = 10, lambda = seq(0.0001, 0.3, length.out = 100)^2
    )
    lassso.mod <- glmnet(
      x = WSmat[rownames(Lam), ][-i, ], y = Lam[-i, f], # Fit LASSO model
      lambda = cv.mod$lambda.min
    )
